...
# Добавочный номер: «+7 910 … доб. 5», «… ext 12», «…,123». Всё, что после
# такого разделителя, к самому номеру не относится.
_EXTENSION = re.compile(r"доб|ext|#|,|;", re.IGNORECASE)
...
def normalize_phone(raw: str) -> str | None:
    ...
    телефонной книги приходят как угодно: «+7 910…», «8-910…», «00 7 910…»,
    со скобками, пробелами и добавочными. Без общего вида не совпадёт ничего,
    и человек просто не найдётся — молча, без всякой ошибки.

    ВАЖНО: клиент нормализует номера точно так же (frontend/src/nativeContacts.ts).
    Любая правка здесь должна повторяться там, иначе хеши перестанут сходиться.
    """
    if not raw:
        return None
    digits = re.sub(r"\D", "", _EXTENSION.split(raw)[0])

    for prefix in _EXIT_PREFIXES:
        # Отрезаем, только если остаток похож на нормальный номер: иначе
        # «00» в середине мусорной строки испортит хороший номер.
        if digits.startswith(prefix) and len(digits) - len(prefix) in (10, 11):
